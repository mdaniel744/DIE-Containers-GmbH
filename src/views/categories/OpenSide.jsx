...
export default function OpenSideCategory() {
  ...
  const products = useMemo(() => {
    let result = allProducts.filter((p) => {
      if (p.container_type !== "Open Side") return false;
    ...
    <div className="pt-20 lg:pt-24 pb-20 min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center gap-1.5 text-xs text-muted-foreground mb-6">
          ...
          <ChevronRight className="w-3 h-3" />
          <span className="text-foreground font-medium">Open Side Container</span>
        </nav>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <p className="text-muted-foreground max-w-2xl">
            Seecontainer mit vollständig öffnbarer Längsseite – maximale Zugänglichkeit für Gabelstaplerbetrieb, sperrige Güter und schnelles Be- und Entladen. In 20 und 40 Fuß erhältlich.
          </p>
          <div className="mt-3 text-sm text-muted-foreground">
            <Link to="/open-side-container-kaufen" className="text-orange-500 hover:underline">
              → Ratgeber: Open Side Container kaufen – Typen, Preise & Einsatzgebiete
            </Link>
          </div>
        </motion.div>
        ...
        {products.length > 0 ? (
          ...
          <div className="text-center py-20">
            ...
            <h3 className="font-heading font-semibold text-lg mb-2">Keine Open Side Container gefunden</h3>
